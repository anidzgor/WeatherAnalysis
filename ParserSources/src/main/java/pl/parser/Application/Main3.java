...
public class Main3 {
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 1; i <= 20; i++) {
            ...
            if (i >= 0 && i <= 9) {
                tempSYNOPBefore = synop.getTemperature(city, "2018-03-" + daySynop + "_0" + (i - 1)).getTemperature();
                tempSYNOPPresent = synop.getTemperature(city, "2018-03-" + daySynop + "_0" + (i)).getTemperature();
            } else {
                if(i == 10)
                    tempSYNOPBefore = synop.getTemperature(city, "2018-03-" + daySynop + "_0" + (i - 1)).getTemperature();
                else {
                    tempSYNOPBefore = synop.getTemperature(city, "2018-03-" + daySynop + "_" + (i - 1)).getTemperature();
                }
                tempSYNOPPresent = synop.getTemperature(city, "2018-03-" + daySynop + "_" + (i)).getTemperature();
            }
